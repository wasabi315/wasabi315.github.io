...
Coverage checking for pattern matching is a static analysis that checks whether a match satisfies two nice properties: _exhaustiveness_ and _non-redundancy_.
...
When there is a redundant clause, it may indicate a mismatch between the programmer's intended case split and what the code actually does.
...
It checks a property called _usefulness_.
Usefulness is defined as follows:

<Box headerText="Definition (Usefulness)">
  A pattern vector $\overrightarrow{p}$ is useful with respect to a pattern
  matrix $P$ (written $\mathcal{U}(P, \overrightarrow{p})$) if: 1. there exists
  a value vector $\overrightarrow{v}$ such that 2. $\overrightarrow{v}$ does not
  match any row of $P$ ($\overrightarrow{v} \npreceq P$), and 3.
  $\overrightarrow{v}$ matches $\overrightarrow{p}$ ($\overrightarrow{v} \preceq
  \overrightarrow{p}$).
</Box>
...
Also, we can check that the 6th row of $Q$ is _not_ useful with respect to the submatrix consisting of rows 1–5 of $Q$.
...
Let $\mathcal{S}$ be this filtering operation (called _specialization_).
...
Now suppose the witness value vector starts with a value of the form `Cons(...)`.
...
Now the first column of both $P$ and $\overrightarrow{p}$ consists entirely of wildcards.
...
That said, most of the work simply simply consisted of translating the operational idea above into Agda, so here I will only highlight a few key parts.
...
In the extended definition, we want witnesses to be _pattern vectors_.
...
<Box headerText="Definition (Extended Usefulness)">
  A pattern vector $\overrightarrow{p}$ is useful (in the extended sense) with
  respect to a pattern matrix $P$ if: 1. there exists a pattern vector
  $\overrightarrow{q}$ such that 2. $\overrightarrow{q}$ is disjoint from $P$
  ($\forall \overrightarrow{v},\ \overrightarrow{v} \preceq \overrightarrow{q}{" "}
  \implies \overrightarrow{v} \npreceq P$) 3. $\overrightarrow{q}$ is subsumed
  by $\overrightarrow{p}$ ($\forall \overrightarrow{v},\ \overrightarrow{v}{" "}
  \preceq \overrightarrow{q} \implies \overrightarrow{v} \preceq \overrightarrow
  {p}$)
</Box>
...
Roughly speaking, we only need to change the places where the original algorithm takes disjunctions (or-patterns, and brute-force splitting for wildcards) because we want to compute all the witnesses.
Instead of taking a disjunction, we take a union and return _all_ witnesses.
...
It is not structurally recursive, so it requires well-founded recursion.
...
1. count pattern size _after fully expanding or-patterns_[^5]
2. do not count wildcards
3. since (2) can break strict decrease in some cases, combine this measure with another decreasing measure (such as the number of columns of the pattern matrix) lexicographically

### Compiling to Haskell with agda2hs

Since the goal is a verified _implementation_ of a coverage checker, I want to make the Agda code runnable.
...
`witness-disjoint-from-P` and `ps-subsumes-witness` contain only proof-related information, so we want to erase them.
...
[^4]: As far as I could find, aside from the concurrent work [Joshua M. Cohen, A Mechanized First-Order Theory of Algebraic Data Types with Pattern Matching](https://doi.org/10.4230/LIPIcs.ITP.2025.5), I was unable to find much. That paper similarly claims that there was little prior work.

[^5]: In practice, we also include the number of or-patterns in the size: if we only count size _after_ expanding them, then the or-pattern step would no longer decrease the measure.
